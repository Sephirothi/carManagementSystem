...
public class ImageUpUtils {
	/**
	 * 文件上传
	 * @param request  HttpServletRequest的请求
	 * @return 返回上传成功的数量暂时版
	 */
	public static int uploadImage(HttpServletRequest request) {
		//用于记录上传文件成功的次数
		int num = 0;
		// 判断是否有文件上传的请求
		...
		if (isMultipart) {
			...
			try {
				...
				while (iter.hasNext()) {
					...
					if (!item.isFormField()&&processUploadedFile(item,savepath)) {
						num++;
					}
				}
	 ...
	 * @return
	 */
	private static boolean processUploadedFile(FileItem item,String savePath) {
...
//				System.out.println(savePath+uuid+type);
				item.write(new File(savePath + uuid+type));
